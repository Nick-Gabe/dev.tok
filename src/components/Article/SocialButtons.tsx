    ...
    <div className="flex flex-col gap-4 justify-end">
      ...
      <div className="dropdown dropdown-top dropdown-end">
        ...
        <ul tabIndex={0} className="dropdown-content menu bg-base-100 rounded-box z-1 w-52 p-2 shadow-sm">
          <li><button className="btn bg-accent m-2" onClick={() => copyToClipboard(article)}>Copy link</button></li>
          <li><button className={`btn bg-accent m-2 block ${typeof navigator.share === 'function' ? '' : 'hidden'}`} onClick={() => navigator.share({ title: article.title, url: article.url })}>Share to</button></li>
          <li><a className="btn bg-error m-2" href={`https://dev.to/report-abuse?url=${encodeURIComponent(article.url)}`} target="_blank" rel="noopener noreferrer">Report</a></li>
        </ul>
      </div>
    </div>
  );
